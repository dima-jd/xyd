...
;(function(window) {
	...
	PieceMaker.prototype._layout = function() {
		this.el.style.backgroundImage = this.el.getAttribute('data-img-XydLab');
		...
		if( this.iteration < this.options.glitchTotalIterations ) {
			this.glitchStateTimeout = setTimeout(function() {
				self.elems.forEach(function(el) {
					if( el.classList.contains('mode--XydLab') ) {
						el.classList.add('mode--Print');
						el.classList.remove('mode--XydLab');
					}
					else {
						el.classList.add('mode--XydLab');
	...
	DOM.switchModeCtrls = {
		'Print' : DOM.switchCtrls.firstElementChild,
		'XydLab' : DOM.switchCtrls.lastElementChild
	};
	...
	DOM.menu = {
		...
		'XydLab' : {
			'wrapper': document.querySelector('.menu--XydLab'),
			'items': document.querySelectorAll('.menu--XydLab > .menu__inner a')
		}
	};
	DOM.overlay = document.querySelector('.overlay');
	// The current mode.
	let mode = 'Print', disablePageFx, isAnimating;
	...
	function init() {
		imagesLoaded(DOM.body, { background: true }, function() {
			...
			pm.loopFx();
			// Glitch effect on some elements (title, contact and XydLabr link) in the page.
			gfx = new GlitchFx(DOM.glitchElems);
			// Split the title, contact and XydLab menu items into spans/letters.
			wordsToLetters();
			// Init/Bind events
			initEvents();
		});
	...
	function wordsToLetters() {
		...
		// Menu items (XydLab mode).
		DOM.menuXydLabItemLetters = [];
		[].slice.call(DOM.menu.XydLab.items).forEach(function(item) {
			charming(item);
			DOM.menuXydLabItemLetters.push([].slice.call(item.querySelectorAll('span')));
		});
	}

	function initEvents() {
		DOM.switchModeCtrls.Print.addEventListener('click', switchMode);
		DOM.switchModeCtrls.XydLab.addEventListener('click', switchMode);
	...
	function switchMode(ev) {
		...
		// mode: Print||XydLab.
		mode = ev.target === DOM.switchModeCtrls.XydLab ? 'XydLab' : 'Print';
		...
		// Change current class on the Printer/XydLabr links.
		DOM.switchModeCtrls[mode === 'XydLab' ? 'Print' : 'XydLab'].classList.remove('switch__item--current');
		...
		// Animate the pieces.
		pm.animatePieces(mode === 'XydLab' ? 'out' : 'in', function() {
			isAnimating = false;
			if( mode === 'Print' ) {
				pm.initTilt();
				pm.loopFx();
	...
	function switchOverlay() {
		...
		anime({
			...
			duration: 800,
			easing: 'linear',
			opacity: mode === 'XydLab' ? 1 : 0
		});
	}

	function switchContent() {
		// Change switchCtrls mode.
		DOM.switchCtrls.classList.remove('mode--' + (mode === 'XydLab' ? 'Print' : 'XydLab'));
	...
	function switchToXydLab() {
		const hidePrint = function(target, callback) {
					let animeOpts = {};
		...
		// Animate the title, contact, menu ctrl and menu items out and show the XydLab mode version of these elements.
		// Title:
		hidePrint('title', function() {
			showXydLab('title');
		});
		// Contact:
		hidePrint('contact', function() {
			showXydLab('contact');
		});
		// Menu ctrl:
		hidePrint('menuCtrl', function() {
			showXydLab('menuCtrl');
		});
		// Menu links:
		hidePrint(DOM.menu['Print'].items, function() {
			DOM.menu['Print'].wrapper.style.display = 'none';
				
			animateLetters(DOM.menuXydLabItemLetters, 'in', {
				delay: function(t,i) {
					return i*30
				},
				begin: function() {
					DOM.menu['XydLab'].wrapper.style.display = 'block';
				}
			});
		});
	}
	...
	function switchToPrint() {
		const showPrint = function(target) {
			  		let animeOpts = {};
		...
		showPrint('menuCtrl');
		// Menu links:
		animateLetters(DOM.menuXydLabItemLetters, 'out', {
			delay: function(t,i,c) {
				return (c-i-1)*10;
			},
			duration: 20,
			complete: function() {
				DOM.menu['XydLab'].wrapper.style.display = 'none';
				DOM.menu['Print'].wrapper.style.display = 'block';
				showPrint(DOM.menu['Print'].items);
			}
		});
	}
